        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setDetailLomba(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto animate-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              {/* Deskripsi lengkap lomba (Peserta, Peralatan, Cara Bermain, Penilaian, dll) */}
              {detailLomba.detailSections && detailLomba.detailSections.length > 0 && (
                <div className="mb-5 space-y-4">
                  {detailLomba.detailSections.map((sec, si) => (
                    <div key={si} className="bg-[#FFF8F0] border border-orange-100 rounded-xl p-4">
                      <h5 className="text-xs font-black text-[#C1272D] uppercase tracking-wider mb-2">{sec.heading}</h5>
                      <ul className="space-y-1.5">
                        {sec.items.map((it, ii) => (
                          <li key={ii} className="text-xs text-gray-700 flex gap-2 items-start leading-relaxed">
              ...
              <div className="mb-5">
                <h4 className="font-bold text-sm mb-2">📋 Peraturan Umum:</h4>
                <ul className="space-y-1.5">
