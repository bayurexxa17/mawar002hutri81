...
import { Handshake, Receipt, Images, Plus, Trash2, Save, PencilLine, RefreshCw, Database, Check, Copy, Music, Archive } from 'lucide-react';
...
export default function AdminPage({ onBack, shared }: Props) {
  ...
  const [tab, setTab] = useState<'peserta' | 'keuangan' | 'inventory' | 'talenta' | 'sponsor' | 'pengeluaran' | 'gallery' | 'panitia' | 'info' | 'rundown' | 'pelaksana' | 'musik' | 'slide' | 'arsip'>('peserta');
  ...
  const persistMusik = async (list: any[]) => {
    ...
    try {
      ...
      const { error } = await supabase.from('musik').insert(list.filter(t => t.url).map((t, i) => ({ judul: t.title, url: t.url, keterangan: t.sub, urutan: i + 1 })));
      ok = !error;
    } catch { /* tabel musik belum ada */ }
    shared.fetchMusic();
    return ok;
  };
  ...
  const addMusik = async () => {
    ...
    const ok = await persistMusik([...shared.musicTracks, t]);
    setMusikForm({ title: '', url: '', ket: '' });
    notify(ok ? '✔ Lagu tersimpan ke Supabase — muncul di semua perangkat' : '⚠️ Tersimpan di perangkat ini saja — tabel "musik" belum ada di Supabase', ok ? 'ok' : 'err');
  };
  const updateMusik = async () => {
    if (!editMusik) return;
    if (!editMusik.url?.trim()) { alert('URL MP3 wajib diisi!'); return; }
    const ok = await persistMusik(shared.musicTracks.map(t => t.id === editMusik.id ? { ...t, title: editMusik.title || t.title, sub: editMusik.ket || t.sub, url: normUrl(editMusik.url) } : t));
    setEditMusik(null);
    notify(ok ? '✔ Lagu diperbarui & tersinkron' : '⚠️ Tersimpan lokal saja — tabel "musik" belum ada', ok ? 'ok' : 'err');
  };
  ...
  // ===== SLIDE DOKUMENTASI handlers =====
  const [slideForm, setSlideForm] = useState({ label: '', title: '', desc: '', img: '', action: '' });
  const [editSlide, setEditSlide] = useState<any | null>(null);

  const submitSlide = async () => {
    const src = editSlide || slideForm;
    if (!src.title.trim() || !src.img.trim()) { alert('Judul dan URL Media wajib diisi!'); return; }
    if (editSlide?.id) {
      shared.setSlideList(prev => prev.map(s => s.id === editSlide.id ? { ...s, label: src.label, title: src.title, desc: src.desc, img: src.img, action: src.action } : s));
      try { await supabase.from('slide_dokumentasi').update({ label: src.label, title: src.title, desc: src.desc, img: src.img, action: src.action }).eq('id', editSlide.id); notify('✔ Slide diperbarui'); }
      catch (e: any) { notify('⚠️ ' + (e?.message || 'Gagal sinkron'), 'err'); }
      setEditSlide(null);
    } else {
      const row = { label: src.label.trim() || 'DOKUMENTASI', title: src.title.trim(), desc: src.desc.trim(), img: src.img.trim(), action: src.action.trim() || 'Lihat', urutan: shared.slideList.length + 1 };
      shared.setSlideList(prev => [...prev, row as any]);
      setSlideForm({ label: '', title: '', desc: '', img: '', action: '' });
      try { await supabase.from('slide_dokumentasi').insert([row]); notify('✔ Slide ditambahkan'); }
      catch (e: any) { notify('⚠️ ' + (e?.message || 'Gagal sinkron'), 'err'); }
    }
    shared.fetchSlides();
  };

  const deleteSlide = async (s: any) => {
    if (!confirm(`Hapus slide "${s.title}"?`)) return;
    shared.setSlideList(prev => prev.filter(x => x.id !== s.id));
    if (s.id) { try { await supabase.from('slide_dokumentasi').delete().eq('id', s.id); notify('✔ Slide dihapus dari Supabase'); } catch (e: any) { notify('⚠️ ' + (e?.message || 'Gagal hapus'), 'err'); } }
    shared.fetchSlides();
  };
  ...
  const submitPanitia = async () => {
    ...
    if (editPanitiaRow?.id) {
      ...
      try {
        await supabase.from('susunan_panitia').update({ jabatan: src.jabatan, nama: src.nama, hp: src.hp }).eq('id', editPanitiaRow.id);
      ...
      try {
        await supabase.from('susunan_panitia').insert([{ ...newRow, urutan: shared.panitiaCore.length + 1 }]);
  ...
  const deletePanitia = async (p: { id?: number; nama: string }) => {
    ...
    if (p.id) {
      try { await supabase.from('susunan_panitia').delete().eq('id', p.id); notify('✔ Panitia dihapus dari Supabase'); }
  ...
  const TABLES_PROBE = ['pendaftar', 'keuangan', 'sponsor', 'donatur', 'kas_rt', 'pengeluaran', 'iuran_warga', 'donasi_cash', 'donasi_online', 'inventory', 'talenta', 'susunan_panitia', 'panitia_pelaksana', 'rundown', 'info_acara', 'slide_dokumentasi', 'arsip_kegiatan', 'gallery', 'musik'];
  ...
  const TABLE_COLS: Record<string, string> = {
    ...
    'kas_rt': 'nama TEXT NOT NULL, jumlah BIGINT DEFAULT 0, keterangan TEXT, jenis TEXT DEFAULT \'kas\'',
    'pengeluaran': 'nama TEXT, keterangan TEXT, jumlah BIGINT DEFAULT 0',
    'iuran_warga': 'nama TEXT NOT NULL, jumlah BIGINT DEFAULT 0, keterangan TEXT',
    'donasi_cash': 'nama TEXT NOT NULL, jumlah BIGINT DEFAULT 0, keterangan TEXT',
    'donasi_online': 'nama TEXT NOT NULL, jumlah BIGINT DEFAULT 0, keterangan TEXT',
    'susunan_panitia': 'urutan INT DEFAULT 0, jabatan TEXT NOT NULL, nama TEXT NOT NULL, hp TEXT',
    ...
    'slide_dokumentasi': 'urutan INT DEFAULT 0, label TEXT, title TEXT, "desc" TEXT, img TEXT, action TEXT DEFAULT \'Lihat\'',
    'arsip_kegiatan': 'tahun TEXT NOT NULL, jenis TEXT DEFAULT \'photo\', title TEXT NOT NULL, url TEXT NOT NULL, deskripsi TEXT',
  ...
  const handleSaveKeuangan = (k: KeuanganEntry) => {
    if (k.id && k.id > 0) {
      ...
      saveToLocal(updated);
      setKeuanganModal(null);
      // Supabase Insert — jenis 'kas' masuk ke tabel `kas_rt`, 'donatur' ke tabel `donatur`
      const isKas = k.jenis === 'kas';
      const isDonatur = k.jenis === 'donatur';
      const target = isKas
        ? supabase.from('kas_rt').insert([{ nama: k.nama, jumlah: k.jumlah, keterangan: k.keterangan, jenis: 'kas' }])
        : isDonatur
  ...
  // ===== ARSIP handlers (realtime → tabel `arsip_kegiatan`) =====
  const [arsipForm, setArsipForm] = useState({ tahun: new Date().getFullYear().toString(), jenis: 'photo', title: '', url: '', deskripsi: '' });
  const [editArsip, setEditArsip] = useState<any | null>(null);

  const submitArsip = async () => {
    const src = editArsip || arsipForm;
    if (!src.tahun.trim() || !src.title.trim() || !src.url.trim()) { alert('Tahun, Judul, dan URL Media wajib diisi!'); return; }
    if (editArsip?.id) {
      shared.setArsipList(prev => prev.map(a => a.id === editArsip.id ? { ...a, tahun: src.tahun, jenis: src.jenis, title: src.title, url: src.url, deskripsi: src.deskripsi } : a));
      try { await supabase.from('arsip_kegiatan').update({ tahun: src.tahun, jenis: src.jenis, title: src.title, url: src.url, deskripsi: src.deskripsi }).eq('id', editArsip.id); notify('✔ Arsip diperbarui'); }
      catch (e: any) { notify('⚠️ ' + (e?.message || 'Gagal sinkron'), 'err'); }
      setEditArsip(null);
    } else {
      const row = { tahun: src.tahun.trim(), jenis: src.jenis, title: src.title.trim(), url: src.url.trim(), deskripsi: src.deskripsi.trim() };
      shared.setArsipList(prev => [{ ...row, id: -(Date.now()) } as any, ...prev]);
      setArsipForm({ tahun: new Date().getFullYear().toString(), jenis: 'photo', title: '', url: '', deskripsi: '' });
      try { await supabase.from('arsip_kegiatan').insert([row]); notify('✔ Arsip ditambahkan'); }
      catch (e: any) { notify('⚠️ ' + (e?.message || 'Gagal sinkron'), 'err'); }
    }
    shared.fetchArsip();
  };

  const deleteArsip = async (a: any) => {
    if (!confirm(`Hapus arsip "${a.title}"?`)) return;
    shared.setArsipList(prev => prev.filter(x => x.id !== a.id));
    if (a.id && a.id > 0) { try { await supabase.from('arsip_kegiatan').delete().eq('id', a.id); notify('✔ Arsip dihapus'); } catch (e: any) { notify('⚠️ Gagal', 'err'); } }
    shared.fetchArsip();
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="flex gap-2 mb-6">
          ...
          <button onClick={() => setTab('slide')} className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-bold transition ${tab === 'slide' ? 'bg-amber-800 text-amber-50 shadow' : 'bg-white text-gray-600 border'}`}>🎞️ Slide ({shared.slideList.length})</button>
          <button onClick={() => setTab('arsip')} className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-bold transition ${tab === 'arsip' ? 'bg-fuchsia-800 text-fuchsia-50 shadow' : 'bg-white text-gray-600 border'}`}><Archive size={15} /> Arsip ({shared.arsipList.length})</button>
          <button onClick={() => setTab('musik')} className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-bold transition ${tab === 'musik' ? 'bg-fuchsia-600 text-white shadow' : 'bg-white text-gray-600 border'}`}><Music size={15} /> Musik ({shared.musicTracks.length})</button>
        ...
        {tab === 'arsip' && (
          <div className="space-y-5">
            <div className="bg-white rounded-2xl shadow-sm border p-5">
              <h4 className="font-bold text-sm text-gray-800 mb-4 flex items-center gap-2"><Archive size={16} className="text-fuchsia-800" /> {editArsip ? 'Edit Arsip Kegiatan' : 'Tambah Arsip Kegiatan (Sejarah HUT RI)'}</h4>
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <input value={(editArsip || arsipForm).tahun} onChange={e => editArsip ? setEditArsip({ ...editArsip, tahun: e.target.value }) : setArsipForm({ ...arsipForm, tahun: e.target.value })} placeholder="Tahun (cth: 2025) *" className="border rounded-xl px-4 py-2.5 text-sm" />
                <select value={(editArsip || arsipForm).jenis} onChange={e => editArsip ? setEditArsip({ ...editArsip, jenis: e.target.value }) : setArsipForm({ ...arsipForm, jenis: e.target.value })} className="border rounded-xl px-4 py-2.5 text-sm bg-white">
                  <option value="photo">📷 Foto Dokumentasi</option>
                  <option value="video">🎬 Video Kegiatan</option>
                  <option value="info">📝 Cerita / Info Singkat</option>
                </select>
                <input value={(editArsip || arsipForm).title} onChange={e => editArsip ? setEditArsip({ ...editArsip, title: e.target.value }) : setArsipForm({ ...arsipForm, title: e.target.value })} placeholder="Judul *" className="border rounded-xl px-4 py-2.5 text-sm sm:col-span-2" />
                <input value={(editArsip || arsipForm).url} onChange={e => editArsip ? setEditArsip({ ...editArsip, url: e.target.value }) : setArsipForm({ ...arsipForm, url: e.target.value })} placeholder="URL Media (jika foto/video)" className="border rounded-xl px-4 py-2.5 text-sm sm:col-span-2" />
                <input value={(editArsip || arsipForm).deskripsi} onChange={e => editArsip ? setEditArsip({ ...editArsip, deskripsi: e.target.value }) : setArsipForm({ ...arsipForm, deskripsi: e.target.value })} placeholder="Deskripsi / Catatan" className="border rounded-xl px-4 py-2.5 text-sm sm:col-span-2" />
              </div>
              <div className="flex gap-2 mt-4">
                <button onClick={submitArsip} className="flex items-center gap-1.5 px-5 py-2.5 bg-fuchsia-800 text-fuchsia-50 text-xs font-bold rounded-xl hover:bg-fuchsia-900 transition shadow"><Save size={14} /> {editArsip ? 'Simpan Perubahan' : 'Tambah Arsip'}</button>
                {editArsip && <button onClick={() => setEditArsip(null)} className="px-5 py-2.5 border text-xs font-bold rounded-xl text-gray-600 hover:bg-gray-50">Batal Edit</button>}
              </div>
            </div>

            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
              <div className="p-4 border-b bg-gray-50/50 flex items-center justify-between">
                <h4 className="font-bold text-sm text-gray-800">Daftar Arsip ({shared.arsipList.length})</h4>
                <button onClick={() => shared.fetchArsip()} className="flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-fuchsia-800"><RefreshCw size={13} /> Refresh</button>
              </div>
              <div className="overflow-x-auto max-h-[55vh] overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800 text-white"><tr>
                    <th className="px-4 py-3 text-left text-xs w-20">Tahun</th><th className="px-4 py-3 text-left text-xs w-24">Jenis</th><th className="px-4 py-3 text-left text-xs">Judul</th><th className="px-4 py-3 text-left text-xs">Media/URL</th><th className="px-4 py-3 text-center text-xs w-24">Aksi</th>
                  </tr></thead>
                  <tbody className="divide-y divide-gray-100">
                    {shared.arsipList.map((a, i) => (
                      <tr key={(a as any).id ?? i} className="hover:bg-fuchsia-50/30 transition">
                        <td className="px-4 py-2.5 font-black text-gray-800 text-xs">{a.tahun}</td>
                        <td className="px-4 py-2.5 text-xs text-gray-500">{a.jenis === 'video' ? '🎬 Video' : a.jenis === 'photo' ? '📷 Foto' : '📝 Info'}</td>
                        <td className="px-4 py-2.5">
                          <div className="font-bold text-sm text-gray-800">{a.title}</div>
                          <div className="text-[10px] text-gray-500 line-clamp-1">{a.deskripsi}</div>
                        </td>
                        <td className="px-4 py-2.5">
                          {a.jenis !== 'info' && a.url && (
                            <a href={a.url} target="_blank" rel="noreferrer" className="text-[10px] text-blue-600 hover:underline truncate block max-w-[200px]">Link Media</a>
                          )}
                        </td>
                        <td className="px-4 py-2.5 text-center">
                          <div className="flex gap-1 justify-center">
                            <button onClick={() => setEditArsip(a)} className="text-[11px] px-2 py-1 bg-blue-50 text-blue-600 rounded-lg font-bold hover:bg-blue-100"><PencilLine size={12} /></button>
                            <button onClick={() => deleteArsip(a)} className="text-[11px] px-2 py-1 bg-red-50 text-red-600 rounded-lg font-bold hover:bg-red-100"><Trash2 size={12} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                {shared.arsipList.length === 0 && <div className="p-8 text-center text-xs text-gray-400">Belum ada arsip.</div>}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'musik' && (
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
              <div className="p-4 border-b bg-gray-50/50 font-bold text-sm text-gray-800">Playlist ({shared.musicTracks.length} lagu) — semua bisa dihapus/diedit</div>
                ...
                {shared.musicTracks.map((t, i) => (
                  <div key={t.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-fuchsia-50/40 transition">
                    ...
                    <div className="flex gap-1">
                      {t.url && <button onClick={() => setEditMusik({ id: t.id, title: t.title, url: t.url, ket: t.sub })} className="text-[11px] px-2 py-1 bg-blue-50 text-blue-600 rounded-lg font-bold hover:bg-blue-100" title="Edit"><PencilLine size={12} /></button>}
                      <button onClick={() => deleteMusik(t.id)} className="text-[11px] px-2 py-1 bg-red-50 text-red-600 rounded-lg font-bold hover:bg-red-100" title="Hapus dari playlist"><Trash2 size={12} /></button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab === 'slide' && (
          <div className="space-y-5">
            <div className="bg-white rounded-2xl shadow-sm border p-5">
              <h4 className="font-bold text-sm text-gray-800 mb-4">🎞️ {editSlide ? 'Edit Slide Dokumentasi' : 'Tambah Slide Dokumentasi (Tampil di beranda bawah Hero)'}</h4>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <input value={(editSlide || slideForm).label} onChange={e => editSlide ? setEditSlide({ ...editSlide, label: e.target.value }) : setSlideForm({ ...slideForm, label: e.target.value })} placeholder="Label (cth: JUARA LOMBA)" className="border rounded-xl px-4 py-2.5 text-sm" />
                <input value={(editSlide || slideForm).title} onChange={e => editSlide ? setEditSlide({ ...editSlide, title: e.target.value }) : setSlideForm({ ...slideForm, title: e.target.value })} placeholder="Judul *" className="border rounded-xl px-4 py-2.5 text-sm" />
                <input value={(editSlide || slideForm).img} onChange={e => editSlide ? setEditSlide({ ...editSlide, img: e.target.value }) : setSlideForm({ ...slideForm, img: e.target.value })} placeholder="URL Gambar / Foto *" className="border rounded-xl px-4 py-2.5 text-sm" />
                <input value={(editSlide || slideForm).desc} onChange={e => editSlide ? setEditSlide({ ...editSlide, desc: e.target.value }) : setSlideForm({ ...slideForm, desc: e.target.value })} placeholder="Deskripsi" className="border rounded-xl px-4 py-2.5 text-sm sm:col-span-2" />
                <input value={(editSlide || slideForm).action} onChange={e => editSlide ? setEditSlide({ ...editSlide, action: e.target.value }) : setSlideForm({ ...slideForm, action: e.target.value })} placeholder="Teks Tombol (cth: Lihat Foto)" className="border rounded-xl px-4 py-2.5 text-sm" />
              </div>
              <div className="flex gap-2 mt-4">
                <button onClick={submitSlide} className="flex items-center gap-1.5 px-5 py-2.5 bg-amber-800 text-amber-50 text-xs font-bold rounded-xl hover:bg-amber-900 transition shadow"><Save size={14} /> {editSlide ? 'Simpan Perubahan' : 'Tambah Slide'}</button>
                {editSlide && <button onClick={() => setEditSlide(null)} className="px-5 py-2.5 border text-xs font-bold rounded-xl text-gray-600 hover:bg-gray-50">Batal Edit</button>}
              </div>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
              <div className="p-4 border-b bg-gray-50/50 flex items-center justify-between">
                <h4 className="font-bold text-sm text-gray-800">Slide Aktif ({shared.slideList.length})</h4>
                <button onClick={() => shared.fetchSlides()} className="flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-amber-800"><RefreshCw size={13} /> Refresh</button>
              </div>
              <div className="grid sm:grid-cols-2 gap-4 p-5">
                {shared.slideList.map(s => (
                  <div key={s.id} className="group border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition flex flex-col">
                    <div className="h-32 bg-gray-100 relative">
                      <img src={s.img} alt={s.title} className="w-full h-full object-cover" loading="lazy" />
                      <span className="absolute top-2 right-2 bg-white/90 text-gray-700 text-[10px] font-bold px-2 py-0.5 rounded-full">{s.label}</span>
                    </div>
                    <div className="p-3 flex-1 flex flex-col">
                      <h5 className="font-bold text-sm text-gray-800 truncate mb-1">{s.title}</h5>
                      <p className="text-[10px] text-gray-500 line-clamp-2 leading-relaxed flex-1">{s.desc}</p>
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                        <span className="text-[10px] font-bold text-amber-800">Btn: {s.action}</span>
                        <div className="flex gap-2">
                          <button onClick={() => setEditSlide(s)} className="text-[11px] font-bold text-blue-600 hover:text-blue-700">Edit</button>
                          <button onClick={() => deleteSlide(s)} className="text-[11px] font-bold text-red-500 hover:text-red-600">Hapus</button>
                        </div>
                      </div>
                    </div>
                  </div>
