...
export default function AdminPage({ onBack, shared }: Props) {
  ...
  const [tab, setTab] = useState<'peserta' | 'keuangan' | 'inventory' | 'talenta' | 'sponsor' | 'pengeluaran' | 'gallery' | 'panitia' | 'info' | 'rundown' | 'pelaksana'>('peserta');
  ...
  const saveInfoAcara = async () => {
    ...
    try {
      const { data } = await supabase.from('info_acara').select('id').limit(1);
      if (data && data.length > 0) {
        await supabase.from('info_acara').update({ tanggal: infoForm.tanggal, waktu: infoForm.waktu, lokasi: infoForm.lokasi, peserta: infoForm.peserta }).eq('id', (data[0] as any).id);
      } else {
        await supabase.from('info_acara').insert([{ tanggal: infoForm.tanggal, waktu: infoForm.waktu, lokasi: infoForm.lokasi, peserta: infoForm.peserta }]);
      }
      notify('✔ Informasi Acara tersimpan & tersinkron');
    ...
    if (k.id && k.id > 0) {
      ...
      Promise.resolve(supabase.from('keuangan').insert([{ nama: k.nama, jenis: k.jenis, jumlah: k.jumlah, keterangan: k.keterangan }]))
        .then((res: any) => {
          ...
          else { shared.fetchKeuangan(); notify(k.jenis === 'pengeluaran' ? '✔ Pengeluaran tercatat — saldo & pemasukan diperbarui realtime' : '✔ Transaksi tersimpan & realtime ke Supabase'); }
        }).catch((e: any) => notify('⚠️ ' + (e?.message || 'Gagal sinkron'), 'err'));
