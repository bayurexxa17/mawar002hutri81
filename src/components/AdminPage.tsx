        ...
        {tab === 'keuangan' && (
          <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
                  ...
                  <tbody>
                    {filteredKeuangan.length === 0 ? <tr><td colSpan={6} className="text-center py-12 text-gray-400">Tidak ada data</td></tr> : filteredKeuangan.map((k, i) => (
                      <tr key={(k.id || '') + '' + i} className={`border-b hover:bg-gray-50 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        ...
                        <td className={`px-3 py-2.5 text-right font-bold ${categorizeKeuangan(k) === 'pengeluaran' ? 'text-red-600' : 'text-green-700'}`}>
                          {categorizeKeuangan(k) === 'pengeluaran' ? `- ${formatRupiah(k.jumlah)}` : formatRupiah(k.jumlah)}
                        </td>
              ...
              {(() => {
                const masuk = filteredKeuangan.filter(k => categorizeKeuangan(k) !== 'pengeluaran').reduce((s, k) => s + (k.jumlah || 0), 0);
