...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchInfoAcara(); fetchRundown(); fetchPelaksana(); fetchMusic(); fetchSlides();
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => { fetchKeuangan(); fetchParticipants(); fetchSponsors(); fetchInventory(); fetchTalenta(); fetchPanitia(); fetchInfoAcara(); fetchRundown(); fetchPelaksana(); fetchSlides(); fetchMusic(); }, 15000);
    return () => { channels.forEach(ch => supabase.removeChannel(ch)); [chP, chT, chI, chInfo, chRd, chPl, chPn1, chPn2, chS, chM].forEach(ch => supabase.removeChannel(ch)); clearInterval(iv); };
  }, [isLive, fetchKeuangan, fetchParticipants, fetchTalenta, fetchSponsors, fetchInventory, fetchInfoAcara, fetchRundown, fetchPelaksana, fetchPanitia, fetchSlides, fetchMusic]);
...
function MainPage({ shared, onAdminClick, onGalleryClick, onInventoryClick }: { shared: SharedData; onAdminClick: () => void; onGalleryClick: () => void; onInventoryClick: () => void }) {
  ...
  const dokumentasiSlides = [
    {
      label: 'JUARA LOMBA',
      title: 'Juara Lomba Tradisional 17 Agustus',
      desc: 'Momen pemenang lomba rakyat, kebersamaan warga, dan ekspresi kemerdekaan Blok Mawar.',
      img: '/images/20260726_091521.jpg',
      action: 'Lihat Foto',
    },
    {
      label: 'DOKUMENTASI LOMBA',
      title: 'Kemeriahan Perlombaan Warga',
      desc: 'Dokumentasi perlombaan anak, remaja, bapak-bapak, ibu-ibu, dan keluarga.',
      img: '/images/20260726_091534.jpg',
      action: 'Lihat Foto',
    },
    {
      label: 'ACARA MALAM PUNCAK',
      title: 'Karnaval & Malam Puncak HUT RI',
      desc: 'Video dan foto acara panggung, talenta anak, hadiah, doorprize, dan ramah tamah warga.',
      img: '/images/Teaser%20HUT%20RI%2081%20Blok%20Mawar%20002.png',
      action: 'Putar / Lihat',
    },
    {
      label: 'VOLLEY BALL',
      title: 'Dokumentasi Turnamen Volley Warga',
      desc: 'Kebersamaan tim putra dan putri warga Mawar 002 dalam kegiatan olahraga kemerdekaan.',
      img: '/images/Team%20Putri%20Mawar1.jpeg',
      action: 'Lihat Foto',
    },
  ];
  const [docIdx, setDocIdx] = useState(0);
  useEffect(() => {
    const iv = setInterval(() => setDocIdx(i => (i + 1) % dokumentasiSlides.length), 4500);
    return () => clearInterval(iv);
  }, [dokumentasiSlides.length]);
      ...
      <section id="dokumentasi-pilihan" className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] bg-white border-y border-gray-100 py-12 sm:py-16 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative rounded-none sm:rounded-3xl overflow-hidden min-h-[380px] sm:min-h-[460px] bg-gray-900 shadow-2xl">
            {dokumentasiSlides.map((slide, i) => (
              <div key={slide.title} className={`absolute inset-0 transition-all duration-700 ease-out ${i === docIdx ? 'opacity-100 scale-100' : 'opacity-0 scale-105 pointer-events-none'}`}>
                <img
                  src={slide.img}
                  alt={slide.title}
                ...
                <div className="relative z-10 h-full flex items-end sm:items-center px-6 sm:px-12 lg:px-16 py-10">
                  <div className="max-w-2xl text-white">
                    ...
                    <div className="mt-6 flex flex-wrap gap-3">
                      ...
                      <button onClick={() => setDocIdx((docIdx + 1) % dokumentasiSlides.length)} className="bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold text-sm px-6 py-3 rounded-full backdrop-blur-sm transition active:scale-95">
                        Slide Berikutnya
                      </button>
                    </div>
                  </div>
            ...
            <div className="absolute top-5 right-6 z-20 bg-black/35 text-white text-xs font-bold rounded-full px-4 py-2 backdrop-blur-sm border border-white/15">
              {docIdx + 1} / {dokumentasiSlides.length}
            </div>
          </div>
        </div>
      </section>
