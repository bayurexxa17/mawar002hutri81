...
const initialSponsorList: SponsorItem[] = [
  { id: 'SP-01', nama: 'Ameera Collections', deskripsi: 'Penyewaan Baju Adat dan Modern', website: '', icon: '👗', warna: 'text-pink-500' },
  { id: 'SP-02', nama: 'warung JEKI', deskripsi: 'Toko Sembako Mawar', icon: '👗', warna: 'text-purple-600' },
  { id: 'SP-03', nama: 'Gorengan Gacor Cak Duro', deskripsi: 'Pesan Telp: +62 822-6831-0077', icon: '🍛', warna: 'text-blue-600' },
];
...
function MainPage({ shared, onAdminClick, onGalleryClick, onInventoryClick }: { shared: SharedData; onAdminClick: () => void; onGalleryClick: () => void; onInventoryClick: () => void }) {
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F5F0] overflow-x-hidden">
      ...
      {/* RUNDOWN */}
      <section id="rundown" className="py-16 px-4 bg-white"><div className="max-w-4xl mx-auto"><div className="text-center mb-8"><h2 className="text-3xl font-black text-gray-900">📋 RUNDOWN ACARA</h2><p className="text-gray-500 mt-1">Jadwal Kegiatan — Senin, 17 Agustus 2026 & Sabtu, 22 Agustus 2026</p></div><div className="flex flex-wrap gap-2 justify-center mb-8"><button onClick={() => { const b = new Blob([rundownText()], { type: 'text/plain' }); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = 'rundown-hutri81-mawar.txt'; a.click(); URL.revokeObjectURL(u); }} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition">📄 Download (TXT)</button><button onClick={() => window.print()} className="flex items-center gap-2 px-4 py-2.5 bg-gray-700 text-white text-xs font-bold rounded-xl hover:bg-gray-800 transition">🖨️ Cetak / Save PDF</button></div><div className="bg-[#F9F5EB] rounded-2xl border p-6"><h4 className="font-bold text-[#C1272D] mb-4 text-sm">☀️ Senin Pagi, 17 Agustus 2026 | Acara Perlombaan</h4><div className="space-y-3 mb-8">{rundownAcaraPerlombaan.map((r, i) => (<div key={i} className="flex items-start gap-4 bg-white rounded-xl p-3 border"><div className="min-w-[60px] text-center"><span className="font-black text-[#C1272D] text-lg">{r.waktu}</span></div><div className="flex-1"><span className="text-lg mr-2">{r.icon}</span><span className="font-semibold text-sm text-gray-800">{r.kegiatan}</span>{r.keterangan && <span className="text-xs text-gray-400 ml-2">({r.keterangan})</span>}</div></div>))}</div><h4 className="font-bold text-[#C1272D] mb-4 text-sm">🌙 MALAM PUNCAK- Sabtu, 22 Agustus 2026</h4><div className="space-y-3">{rundownMalam.map((r, i) => (<div key={i} className="flex items-start gap-4 bg-white rounded-xl p-3 border"><div className="min-w-[60px] text-center"><span className="font-black text-[#C1272D] text-lg">{r.waktu}</span></div><div className="flex-1"><span className="text-lg mr-2">{r.icon}</span><span className="font-semibold text-sm text-gray-800">{r.kegiatan}</span>{r.keterangan && <span className="text-xs text-gray-400 ml-2">({r.keterangan})</span>}</div></div>))}</div></div></div></section>
