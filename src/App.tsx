...
export interface SlideItem { id?: number; label: string; title: string; desc: string; img: string; action: string; urutan: number; }
...
export interface SharedData {
  ...
  slideList: SlideItem[];
  setSlideList: React.Dispatch<React.SetStateAction<SlideItem[]>>;
  ...
  arsipList: ArsipItem[];
  setArsipList: React.Dispatch<React.SetStateAction<ArsipItem[]>>;
  fetchArsip: () => Promise<void>;
...
export function categorizeKeuangan(k: KeuanganEntry): 'cash' | 'kas' | 'donatur' | 'sponsor' | 'donasi' | 'pengeluaran' | 'transfer' {
  ...
  // JENIS adalah penentu utama — diperiksa lebih dulu agar keterangan
  // (mis. "Transfer seabank..." pada setoran Kas RT) tidak salah menggolongkan.
  ...
  if (j.includes('kas') || ket.includes('kas rt') || ket.includes('kasrt') || ket === 'kas') return 'kas';
  if (j.includes('iuran') || j.includes('cash')) return 'cash';
  // Baru setelah jenis tidak dikenal, lihat keterangan untuk transfer
...
// ================== PEMUTAR MUSIK (VERSI FINAL & AMAN) ==================
...
const loadSavedTracks = (): Track[] => {
  try {
    const s = localStorage.getItem(LS_MUSIC);
    if (!s) return [];
    const raw = JSON.parse(s) as Track[];
    const seen = new Set<string>();
    return raw
      .filter(t => t && t.kind === 'audio' && !!t.url)
      .filter(t => {
        const key = normUrl(String(t.url)).split('?')[0];
...
function MusicPlayer({ custom }: { custom: Track[] }) {
  const tracks = Array.isArray(custom) ? custom.filter(t => t?.url) : [];
  ...
  const ensureAudio = () => {
    if (!audioRef.current) {
      ...
      a.addEventListener('error', () => {
        setAudioErr('Audio gagal dimuat. Pastikan URL MP3 publik/direct link.');
        setPlaying(false);
      });
      audioRef.current = a;
    }
  ...
  const playAt = async (i: number) => {
    ...
    a.volume = muted ? 0 : vol / 100;
    try {
      await a.play();
      setIdx(i);
      setPlaying(true);
    } catch {
      setAudioErr('Browser memblokir pemutaran. Klik tombol play sekali lagi.');
      setPlaying(false);
    }
  };

  const pause = () => { audioRef.current?.pause(); setPlaying(false); };
  const toggle = () => { if (playing) pause(); else playAt(idx); };
  const goNext = () => playAt((idx + 1) % tracks.length);
  const goPrev = () => playAt((idx - 1 + tracks.length) % tracks.length);

  useEffect(() => {
    if (audioRef.current) audioRef.current.volume = muted ? 0 : vol / 100;
  }, [vol, muted]);
  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 flex flex-col items-start gap-3">
      {open && (
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-72 animate-in overflow-hidden">
          <div className="bg-gradient-to-r from-[#C1272D] to-[#8B1A1A] px-4 py-3 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full bg-white/15 border border-white/25 flex items-center justify-center flex-shrink-0 ${playing ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
            ...
            <button onClick={() => setMinimized(m => !m)} className="text-white/70 hover:text-white transition flex-shrink-0" title={minimized ? 'Perbesar panel' : 'Minimize panel'}>
          ...
          {!minimized && (
            <>
              {audioErr && <div className="px-4 py-2 bg-red-50 border-b border-red-100 text-[10px] font-semibold text-red-600 leading-relaxed">⚠️ {audioErr}</div>}
              ...
              <div className="max-h-44 overflow-y-auto">
                ...
                {tracks.map((t, i) => (
                  <button key={t.id} type="button" className={`w-full text-left group flex items-center gap-2 px-4 py-2 cursor-pointer transition ${i === idx ? 'bg-red-50' : 'hover:bg-gray-50'}`} onClick={() => playAt(i)}>
                    ...
                    <span className="text-[9px] font-bold text-gray-300 uppercase">MP3</span>
                  </button>
                ))}
              </div>
              <div className="px-4 py-2.5 bg-gray-50 border-t border-gray-100">
                <p className="text-[9px] text-gray-400 leading-relaxed">Tambah / edit / hapus lagu via <strong>Panel Panitia → 🎵 Musik</strong>.</p>
              </div>
            </>
          )}
        </div>
      ...
      <button
        type="button"
        onClick={() => { if (!open) { setOpen(true); setMinimized(true); } else toggle(); }}
...
export default function App() {
  const [page, setPage] = useState<'main' | 'admin' | 'gallery' | 'inventory' | 'arsip'>('main');
  ...
  const fetchKeuangan = useCallback(async () => {
    ...
    // Baca semua varian nama tabel (underscore & spasi) sesuai yang ada di Supabase Anda
    const sources: [string, string, number][] = [
      ['keuangan', 'donasi', 0],
      ['sponsor', 'sponsor', 1000000],
      ['iuran_warga', 'iuran', 2000000],
      ['iuran warga', 'iuran', 2400000],
      ['donasi', 'donasi', 3000000],
      ['donasi_cash', 'cash', 4000000],
      ['donasi cash', 'cash', 4400000],
      ['donasi_online', 'donasi', 5000000],
      ['donasi online', 'donasi', 5400000],
  ...
  const fetchTalenta = useCallback(async () => {
    try {
      // Tanpa .order() — kolom 'no' mungkin belum ada; urutkan di sisi aplikasi
      const { data } = await supabase.from('talenta').select('*');
      if (data && data.length > 0) {
        const rows = [...data].sort((a: any, b: any) => (Number(a.no ?? a.urutan) || Number(a.id) || 0) - (Number(b.no ?? b.urutan) || Number(b.id) || 0));
        setTalentaList(rows.map((r: any) => ({
          id: r.id,
  ...
  // Default slide dari instruksi Anda (fallback)
  const defaultSlides: SlideItem[] = [
    { label: 'JUARA LOMBA', title: 'Juara Lomba Tradisional 17 Agustus', desc: 'Momen pemenang lomba rakyat, kebersamaan warga, dan ekspresi kemerdekaan Blok Mawar.', img: '/images/20260726_091521.jpg', action: 'Lihat Foto', urutan: 1 },
    { label: 'DOKUMENTASI LOMBA', title: 'Kemeriahan Perlombaan Warga', desc: 'Dokumentasi perlombaan anak, remaja, bapak-bapak, ibu-ibu, dan keluarga.', img: '/images/20260726_091534.jpg', action: 'Lihat Foto', urutan: 2 },
    { label: 'ACARA MALAM PUNCAK', title: 'Karnaval & Malam Puncak HUT RI', desc: 'Video dan foto acara panggung, talenta anak, hadiah, doorprize, dan ramah tamah warga.', img: '/images/Teaser%20HUT%20RI%2081%20Blok%20Mawar%20002.png', action: 'Putar / Lihat', urutan: 3 },
    { label: 'VOLLEY BALL', title: 'Dokumentasi Turnamen Volley Warga', desc: 'Kebersamaan tim putra dan putri warga Mawar 002 dalam kegiatan olahraga kemerdekaan.', img: '/images/Team%20Putri%20Mawar1.jpeg', action: 'Lihat Foto', urutan: 4 },
  ];
  const [slideList, setSlideList] = useState<SlideItem[]>(defaultSlides);
  const [arsipList, setArsipList] = useState<ArsipItem[]>([]);

  const fetchSlides = useCallback(async () => {
    try {
      const { data, error } = await supabase.from('slide_dokumentasi').select('*');
      if (!error && data && data.length > 0) {
        const rows = [...data].sort((a: any, b: any) => (Number(a.urutan) || Number(a.id) || 0) - (Number(b.urutan) || Number(b.id) || 0));
        setSlideList(rows.map((r: any) => ({ id: r.id, label: r.label || '', title: r.title || '', desc: r.desc || r.deskripsi || '', img: r.img || r.url || '', action: r.action || 'Lihat', urutan: Number(r.urutan) || 0 })));
      }
  ...
  const fetchArsip = useCallback(async () => {
    try {
      const { data, error } = await supabase.from('arsip_kegiatan').select('*').order('tahun', { ascending: false });
      if (!error && data) {
        setArsipList(data.map((r: any) => ({ id: r.id, tahun: r.tahun, jenis: r.jenis, title: r.title, url: r.url, deskripsi: r.deskripsi })));
      }
    } catch {}
  }, []);
  ...
  const fetchPanitia = useCallback(async () => {
    // Coba `susunan_panitia` dulu (tabel Anda), lalu fallback ke `panitia`
    for (const tbl of ['susunan_panitia', 'panitia']) {
      try {
        const { data, error } = await supabase.from(tbl).select('*');
        if (!error && data && data.length > 0) {
          const rows = [...data].sort((a: any, b: any) => (Number(a.urutan) || Number(a.id) || 0) - (Number(b.urutan) || Number(b.id) || 0));
          setPanitiaCore(rows.map((r: any) => ({ id: r.id, jabatan: r.jabatan || '', nama: r.nama || '', hp: r.hp || '' })));
          console.info(`[Supabase] ${tbl}:`, rows.length, 'baris');
          return;
        }
      } catch { /* coba tabel berikutnya */ }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchInfoAcara(); fetchRundown(); fetchPelaksana(); fetchMusic(); fetchSlides(); fetchArsip();
  ...
  // Realtime: langganan SEMUA tabel
  useEffect(() => {
    if (!isLive) return;
    const channels = ['keuangan', 'sponsor', 'iuran_warga', 'iuran warga', 'donasi', 'donasi_cash', 'donasi cash', 'donasi_online', 'donasi online', 'pengeluaran', 'kas_rt', 'kas rt', 'donatur'].map(tbl =>
    ...
    // Coba dua nama tabel susunan_panitia dan panitia
    const chPn1 = supabase.channel('rt-panitia').on('postgres_changes', { event: '*', schema: 'public', table: 'panitia' }, () => { fetchPanitia(); }).subscribe();
    const chPn2 = supabase.channel('rt-susunan-panitia').on('postgres_changes', { event: '*', schema: 'public', table: 'susunan_panitia' }, () => { fetchPanitia(); }).subscribe();
    
    const chS = supabase.channel('rt-slide').on('postgres_changes', { event: '*', schema: 'public', table: 'slide_dokumentasi' }, () => { fetchSlides(); }).subscribe();
    const chM = supabase.channel('rt-musik').on('postgres_changes', { event: '*', schema: 'public', table: 'musik' }, () => { fetchMusic(); }).subscribe();
    const chAr = supabase.channel('rt-arsip').on('postgres_changes', { event: '*', schema: 'public', table: 'arsip_kegiatan' }, () => { fetchArsip(); }).subscribe();
    
    const iv = setInterval(() => { fetchKeuangan(); fetchParticipants(); fetchSponsors(); fetchInventory(); fetchTalenta(); fetchPanitia(); fetchInfoAcara(); fetchRundown(); fetchPelaksana(); fetchSlides(); fetchMusic(); fetchArsip(); }, 15000);
    return () => { channels.forEach(ch => supabase.removeChannel(ch)); [chP, chT, chI, chInfo, chRd, chPl, chPn1, chPn2, chS, chM, chAr].forEach(ch => supabase.removeChannel(ch)); clearInterval(iv); };
  }, [isLive, fetchKeuangan, fetchParticipants, fetchTalenta, fetchSponsors, fetchInventory, fetchInfoAcara, fetchRundown, fetchPelaksana, fetchPanitia, fetchSlides, fetchMusic, fetchArsip]);
  ...
  useEffect(() => {
    const check = () => {
      ...
      else if (h === '#arsip' || h === '#/arsip') setPage('arsip');
      else setPage('main');
    };
    check();
  ...
  const sharedData: SharedData = {
    ...
    slideList, setSlideList, fetchSlides,
    arsipList, setArsipList, fetchArsip
  };
  ...
  if (page === 'arsip') return <><ArsipPage key="arsip-page" onBack={goMain} shared={sharedData} /><MusicPlayer custom={musicTracks} /></>;
  return (
    <>
      <MainPage
        key="main-page"
        ...
        onArsipClick={() => { setPage('arsip'); window.location.hash = '#arsip'; }}
      />
...
// ================== MAIN PAGE ==================
function MainPage({ shared, onAdminClick, onGalleryClick, onInventoryClick, onArsipClick }: { shared: SharedData; onAdminClick: () => void; onGalleryClick: () => void; onInventoryClick: () => void; onArsipClick: () => void }) {
  ...
  const navLinks = [
    ...
    { id: 'arsip-link', label: 'Arsip', action: onArsipClick },
    { id: 'gallery-link', label: 'Galeri', action: onGalleryClick },
  ];

  const [docIdx, setDocIdx] = useState(0);
  useEffect(() => {
    if (shared.slideList.length === 0) return;
    const iv = setInterval(() => setDocIdx(i => (i + 1) % shared.slideList.length), 4500);
    return () => clearInterval(iv);
  }, [shared.slideList.length]);
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F5F0] overflow-x-hidden">
      ...
      {/* FULL-WIDTH SLIDE DOKUMENTASI */}
      <section id="dokumentasi-pilihan" className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] bg-white border-y border-gray-100 py-12 sm:py-16 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-6">
            <div>
              <p className="text-[11px] font-black tracking-[0.35em] text-[#C1272D] uppercase mb-2">Dokumentasi Pilihan</p>
              <h2 className="text-3xl sm:text-4xl font-black text-gray-950 tracking-tight flex items-center gap-3">
                <span>🏆</span> Juara & Dokumentasi Acara
              </h2>
              <p className="text-sm text-gray-500 mt-2">Foto para juara, perlombaan warga, dan video Malam Puncak HUT RI ke-81.</p>
            </div>
            <button onClick={onGalleryClick} className="text-xs font-bold text-[#C1272D] hover:underline self-start lg:self-auto">
              Klik media untuk melihat galeri lengkap →
            </button>
          </div>

          <div className="relative rounded-none sm:rounded-3xl overflow-hidden min-h-[380px] sm:min-h-[460px] bg-gray-900 shadow-2xl">
            {shared.slideList.map((slide, i) => (
              <div key={slide.id || i} className={`absolute inset-0 transition-all duration-700 ease-out ${i === docIdx ? 'opacity-100 scale-100' : 'opacity-0 scale-105 pointer-events-none'}`}>
                <img
                  src={slide.img}
                  alt={slide.title}
                  className="absolute inset-0 w-full h-full object-cover"
                  onError={(e) => { e.currentTarget.src = 'https://images.pexels.com/photos/33807994/pexels-photo-33807994.jpeg?auto=compress&cs=tinysrgb&w=1600'; }}
                />
                <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/35 to-black/10" />
                <div className="relative z-10 h-full flex items-end sm:items-center px-6 sm:px-12 lg:px-16 py-10">
                  <div className="max-w-2xl text-white">
                    <span className="inline-flex items-center bg-white text-[#C1272D] rounded-full px-4 py-1.5 text-[11px] font-black uppercase tracking-wider shadow-md mb-4">
                      {slide.label}
                    </span>
                    <h3 className="text-3xl sm:text-5xl font-black leading-tight drop-shadow-lg">{slide.title}</h3>
                    <p className="mt-4 text-white/80 text-sm sm:text-base leading-relaxed max-w-xl">{slide.desc}</p>
                    <div className="mt-6 flex flex-wrap gap-3">
                      <button onClick={onGalleryClick} className="bg-[#C1272D] hover:bg-red-700 text-white font-bold text-sm px-6 py-3 rounded-full shadow-lg transition active:scale-95">
                        🔍 {slide.action || 'Lihat'}
                      </button>
                      <button onClick={() => setDocIdx((docIdx + 1) % shared.slideList.length)} className="bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold text-sm px-6 py-3 rounded-full backdrop-blur-sm transition active:scale-95">
                        Slide Berikutnya
                      </button>
                    </div>
                  </div>
            ...
            <div className="absolute bottom-5 left-6 sm:left-12 lg:left-16 z-20 flex items-center gap-2">
              {shared.slideList.map((_, i) => (
                <button key={i} onClick={() => setDocIdx(i)} className={`h-2 rounded-full transition-all ${i === docIdx ? 'w-10 bg-white' : 'w-2 bg-white/40 hover:bg-white/70'}`} aria-label={`Slide ${i + 1}`} />
              ))}
            </div>
            <div className="absolute top-5 right-6 z-20 bg-black/35 text-white text-xs font-bold rounded-full px-4 py-2 backdrop-blur-sm border border-white/15">
              {docIdx + 1} / {shared.slideList.length}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            ...
            <div className="bg-white text-gray-900 rounded-xl p-4 border border-gray-200 relative overflow-hidden">
              ...
              <div className="text-[10px] text-gray-400 mt-1" title="Terhitung dari transaksi berjenis 'Kas RT'">{kasList.length} setoran • jenis: Kas RT</div>
            </div>
            <div className="bg-white text-gray-900 rounded-xl p-4 border border-gray-200">
