...
import { lombaList, panitiaList, budgetRows, budgetDetails, formatRupiah, rundownPagi, rundownMalam } from './data/siteData';
import { supabase } from './utils/supabaseClient';
import qrisImage from './assets/qris-aulia.png';

// ================== SHARED TYPES ==================
export interface Participant { id: string; name: string; rt: string; hp: string; lomba: string[]; catatan: string; waktu: string; dbId?: number; }
export interface KeuanganEntry { id?: number; nama: string; jenis: string; jumlah: number; keterangan: string; is_anon?: boolean; created_at?: string; }
export interface DonorEntry { id: string; name: string; alamat: string; jumlah: number; pesan: string; waktu: string; isAnon: boolean; jenis: string; }

const defaultParticipants: Participant[] = [
  { id: 'MWR81-0001', name: 'Fatimah Az Zahra', rt: 'RT 002 / Blok Mawar', hp: '081234567890', lomba: ['Makan Kerupuk','Balap Kelereng'], catatan: 'Peserta resmi terdaftar', waktu: '10/06/2026, 09:00:00' },
  { id: 'MWR81-0002', name: 'Ameera Hanania R', rt: 'RT 002 / Blok Mawar', hp: '081234567891', lomba: ['Fashion Week Daster','Estafet Penguin Anak'], catatan: 'Peserta resmi terdaftar', waktu: '10/06/2026, 09:05:00' },
];

// ================== ROOT APP — SHARED STATE ==================
export default function App() {
  const [page, setPage] = useState<'main' | 'admin' | 'gallery'>('main');

  // ===== SHARED STATE (single source of truth) =====
  const [participants, setParticipants] = useState<Participant[]>(defaultParticipants);

  // Keuangan: load dari localStorage saat init agar tidak hilang saat refresh
  const [keuanganList, setKeuanganList] = useState<KeuanganEntry[]>(() => {
    try { const saved = localStorage.getItem('hutri81-keuangan'); return saved ? JSON.parse(saved) : []; } catch { return []; }
  });
  const [totalDana, setTotalDana] = useState(() => {
    try { const saved = localStorage.getItem('hutri81-keuangan'); if (saved) { const arr = JSON.parse(saved); return arr.reduce((s: number, k: any) => s + (k.jumlah || 0), 0); } return 0; } catch { return 0; }
  });
  const [isLive, setIsLive] = useState(true);
  const [lastRefresh, setLastRefresh] = useState(new Date());
  const [newRowIds, setNewRowIds] = useState<Set<string>>(new Set());

  // ===== SIMPLE DATA LOGIC — no complex merge/overwrite =====
  // Try Supabase ONCE on load. After that, ALL data lives in local state.
  // Supabase insert is fire-and-forget (works when connected, ignored when not).
  ...
  // Auto-save keuangan ke localStorage setiap kali berubah (backup)
  useEffect(() => {
    try { localStorage.setItem('hutri81-keuangan', JSON.stringify(keuanganList)); } catch {}
  }, [keuanganList]);

  // Fetch ONCE on mount
  useEffect(() => { fetchParticipants(); fetchKeuangan(); }, []);
  ...
  // ===== SHARED PROPS =====
  const sharedData = { participants, keuanganList, totalDana, isLive, lastRefresh, newRowIds, setIsLive, fetchParticipants, fetchKeuangan, setNewRowIds, setParticipants, setKeuanganList, setTotalDana, setLastRefresh };
  ...
  return <MainPage key="main-page" shared={sharedData} onAdminClick={() => { setPage('admin'); window.location.hash = '#admin'; }} onGalleryClick={() => { setPage('gallery'); window.location.hash = '#gallery'; }} />;
}

// ================== SHARED DATA TYPE ==================
export interface SharedData {
  participants: Participant[]; keuanganList: KeuanganEntry[]; totalDana: number; isLive: boolean;
  lastRefresh: Date; newRowIds: Set<string>; setIsLive: (v: boolean) => void;
  fetchParticipants: () => Promise<void>; fetchKeuangan: () => Promise<void>;
  setNewRowIds: React.Dispatch<React.SetStateAction<Set<string>>>; setParticipants: React.Dispatch<React.SetStateAction<Participant[]>>;
  setKeuanganList: React.Dispatch<React.SetStateAction<KeuanganEntry[]>>; setTotalDana: React.Dispatch<React.SetStateAction<number>>;
  setLastRefresh: React.Dispatch<React.SetStateAction<Date>>;
}

// ================== MAIN PAGE ==================
function MainPage({ shared, onAdminClick, onGalleryClick }: { shared: SharedData; onAdminClick: () => void; onGalleryClick: () => void }) {
  ...
  const handleDonasi = (e: React.FormEvent) => {
    ...
    const newK: KeuanganEntry = { nama: donorName, jenis: donasiForm.metode === 'cash' ? 'cash' : 'donasi', jumlah, keterangan: `[${metodeLabel}] ${donasiForm.alamat}`, is_anon: donasiForm.isAnon, created_at: new Date().toISOString() };
    ...
    try { localStorage.setItem('hutri81-keuangan', JSON.stringify(updated)); } catch {} // LANGSUNG simpan
  ...
  const navLinks = [{ id: 'beranda', label: 'Beranda' },{ id: 'tentang', label: 'Ringkasan' },{ id: 'lomba', label: 'Lomba' },{ id: 'rundown', label: 'Jadwal' },{ id: 'panitia', label: 'Panitia' },{ id: 'gallery-link', label: 'Galeri', action: onGalleryClick }];
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F5F0] overflow-x-hidden">
      ...
      {/* DONASI — Enhanced with payment method + live transaction feed */}
      <section id="donasi" className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-gray-900">❤️ Donasi & Pembayaran</h2>
            <p className="text-gray-500 mt-1">Bayar via QRIS DANA atau Transfer SeaBank — langsung tersinkron real-time</p>
          </div>

          <div className="grid lg:grid-cols-2 gap-6 mb-8">
            {/* LEFT: Form Konfirmasi */}
            <div className="bg-[#F9F5EB] rounded-2xl border p-6">
              <h3 className="font-bold text-lg text-[#C1272D] mb-4">❤️ Konfirmasi Pembayaran</h3>
              <form onSubmit={handleDonasi} className="space-y-3">
                {/* Metode Pembayaran */}
                <div>
                  <label className="text-xs font-semibold text-gray-600 block mb-2">Metode Pembayaran *</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[{v:'qris_dana',l:'QRIS DANA',i:'💳'},{v:'transfer_seabank',l:'Transfer SeaBank',i:'🏦'},{v:'cash',l:'Cash / Tunai',i:'💵'}].map(m=>(
                      <button key={m.v} type="button" onClick={()=>setDonasiForm({...donasiForm,metode:m.v})}
                        className={`py-2.5 px-2 rounded-xl text-xs font-bold border-2 transition-all text-center ${donasiForm.metode===m.v ? 'border-[#C1272D] bg-red-50 text-[#C1272D]' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}>
                        <div className="text-lg mb-0.5">{m.i}</div>{m.l}
                      </button>
                    ))}
                  </div>
                </div>
                <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={donasiForm.isAnon} onChange={e => setDonasiForm({...donasiForm,isAnon:e.target.checked})} className="rounded" /> Hamba Allah (Anonim)</label>
                {!donasiForm.isAnon && <input required value={donasiForm.name} onChange={e => setDonasiForm({...donasiForm,name:e.target.value})} placeholder="Nama Pembayar / Donatur" className="w-full border rounded-xl px-4 py-2.5 text-sm bg-white" />}
                <input required value={donasiForm.alamat} onChange={e => setDonasiForm({...donasiForm,alamat:e.target.value})} placeholder="Alamat / Blok Rumah" className="w-full border rounded-xl px-4 py-2.5 text-sm bg-white" />
                <input required type="number" value={donasiForm.jumlah} onChange={e => setDonasiForm({...donasiForm,jumlah:e.target.value})} placeholder="Jumlah (Rp)" className="w-full border rounded-xl px-4 py-2.5 text-sm bg-white" />
                <button type="submit" className="w-full bg-[#C1272D] text-white font-bold py-3 rounded-xl hover:bg-red-700 transition text-sm">✅ Konfirmasi Pembayaran</button>
                <p className="text-[10px] text-gray-400 text-center">⚡ Konfirmasi langsung tersinkron real-time ke Panel Panitia & Dashboard</p>
              </form>
            </div>

            {/* RIGHT: QRIS + Bank Info */}
            <div className="bg-[#F9F5EB] rounded-2xl border p-6 text-center flex flex-col items-center justify-center">
              <h4 className="font-bold text-lg mb-4">📱 QRIS & Rekening Resmi</h4>
              <div className="bg-white rounded-2xl border shadow-sm p-3 mb-4 inline-block">
                <img src={qrisImage} alt="QRIS Aulia Komari" className="w-48 h-48 object-contain rounded-xl" />
              </div>
              <p className="font-bold text-[#C1272D] text-sm mb-1">AULIA KOMARI</p>
              <p className="text-gray-500 text-xs mb-3">Bendahara HUT RI ke-81</p>
              <div className="text-left w-full max-w-xs space-y-2">
                <div className="bg-white rounded-xl p-3 border flex items-center gap-3 hover:shadow-md transition cursor-pointer" onClick={()=>{navigator.clipboard.writeText('901592977740');alert('Nomor SeaBank disalin!')}}>
                  <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">🏦</div>
                  <div className="flex-1 min-w-0"><div className="text-xs font-bold text-gray-800">SeaBank</div><div className="font-mono text-sm text-gray-700 select-all">901592977740</div></div>
                  <span className="text-[10px] text-blue-600 font-bold">Salin</span>
                </div>
                <div className="bg-white rounded-xl p-3 border flex items-center gap-3 hover:shadow-md transition cursor-pointer" onClick={()=>{navigator.clipboard.writeText('081364755007');alert('Nomor DANA disalin!')}}>
                  <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">💳</div>
                  <div className="flex-1 min-w-0"><div className="text-xs font-bold text-gray-800">DANA</div><div className="font-mono text-sm text-gray-700 select-all">081364755007</div></div>
                  <span className="text-[10px] text-blue-600 font-bold">Salin</span>
                </div>
              </div>
            </div>
      ...
      {/* TRANSAKSI KEUANGAN REAL-TIME — dipindah ke atas Susunan Panitia */}
      <section id="transaksi" className="py-16 px-4 bg-[#F5F5F0]">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-black text-gray-900">💰 Transaksi Keuangan Real-Time</h2>
            <p className="text-gray-500 mt-1 text-sm">QRIS DANA & Transfer Bank — tersinkron otomatis</p>
          </div>
          <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h4 className="font-bold text-white text-sm">💰 Riwayat Transaksi Real-Time</h4>
                <div className="flex items-center gap-1.5 bg-white/20 rounded-full px-2.5 py-1">
                  <span className="relative flex h-2 w-2"><span className="animate-ping absolute h-full w-full rounded-full bg-white opacity-75"></span><span className="relative rounded-full h-2 w-2 bg-white"></span></span>
                  <span className="text-[10px] text-white/90 font-bold">LIVE</span>
                </div>
              </div>
              <div className="text-right">
                <div className="text-white font-black text-lg">{formatRupiah(totalDana)}</div>
                <div className="text-white/60 text-[10px]">Total Terkumpul</div>
              </div>
            </div>
            {shared.keuanganList.length === 0 ? (
              <div className="p-8 text-center text-gray-400"><div className="text-3xl mb-2">📭</div><p className="text-sm font-semibold">Belum ada transaksi</p><p className="text-xs mt-1">Transaksi via QRIS, Transfer, atau Cash akan muncul di sini secara real-time</p></div>
            ) : (
              <div className="divide-y max-h-[400px] overflow-y-auto">
                {shared.keuanganList.slice(0, 30).map((k, i) => (
                  <div key={(k.id||'') + '' + i} className="px-5 py-3 flex items-center gap-4 hover:bg-gray-50 transition">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${
                      (k.keterangan||'').includes('QRIS') ? 'bg-blue-100' : (k.keterangan||'').includes('SeaBank') ? 'bg-cyan-100' : (k.jenis==='iuran') ? 'bg-amber-100' : (k.jenis==='sponsor') ? 'bg-purple-100' : (k.jenis==='cash') ? 'bg-green-100' : 'bg-pink-100'
                    }`}>
                      {(k.keterangan||'').includes('QRIS') ? '💳' : (k.keterangan||'').includes('SeaBank') ? '🏦' : (k.jenis==='cash') ? '💵' : (k.jenis==='iuran') ? '💰' : (k.jenis==='sponsor') ? '🏢' : '❤️'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm text-gray-900">{k.is_anon ? '🙈 Hamba Allah' : k.nama}</div>
                      <div className="text-xs text-gray-400 truncate">{k.keterangan || k.jenis} • {k.created_at ? new Date(k.created_at).toLocaleString('id-ID') : '-'}</div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="font-black text-green-600 text-sm">+{formatRupiah(k.jumlah)}</div>
                      <div className="text-[10px] text-green-500">✅ Terkonfirmasi</div>
                    </div>
                  </div>
                ))}
              </div>
            )}
            <div className="px-5 py-2.5 bg-gray-50 border-t text-[11px] text-gray-400 flex justify-between">
              <span>{shared.keuanganList.length} transaksi tercatat</span>
              <span>↔ Sinkron Panel Panitia • Supabase</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="peserta" className="relative">
        ...
        </div><div className="grid sm:grid-cols-3 gap-4 mt-6"><div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm"><div className="flex items-center gap-2 mb-2"><span className="w-8 h-8 bg-yellow-100 rounded-lg flex items-center justify-center text-lg">⚡</span><h4 className="font-bold text-sm text-gray-800">Filter Ketat Aktif</h4></div><p className="text-xs text-gray-500 leading-relaxed">Otomatis membuang baris RT kosong dan nomor uji coba spam.</p></div><div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm"><div className="flex items-center gap-2 mb-2"><span className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center text-lg">🔄</span><h4 className="font-bold text-sm text-gray-800">Tempo Reload</h4></div><p className="text-xs text-gray-500 leading-relaxed">Data terbaru muncul di atas dengan highlight kuning otomatis.</p></div><div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm"><div className="flex items-center gap-2 mb-2"><span className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center text-lg">📤</span><h4 className="font-bold text-sm text-gray-800">Export & Share</h4></div><p className="text-xs text-gray-500 leading-relaxed">Unduh CSV untuk laporan panitia. Siap print.</p></div></div></div>
      </section>
      ...
      {/* FOOTER */}
      <footer className="bg-[#1a1a1a] text-white py-12 px-4"><div className="max-w-6xl mx-auto text-center"><div className="flex items-center justify-center gap-3 mb-4"><div className="w-12 h-12 rounded-full bg-yellow-400 flex items-center justify-center font-black text-[#C1272D] text-lg shadow">81</div><div className="text-left"><div className="font-bold text-lg">HUT RI Ke-81</div><div className="text-xs text-gray-400">Perumahan Ciptaland Blok Mawar</div></div></div><p className="text-gray-400 text-sm mb-2">RT 002 / RW 014</p><p className="text-gray-500 text-xs mb-4">📧 panitiahutri81.mawar002@gmail.com</p><div className="flex items-center justify-center gap-4 mb-6"><button onClick={onGalleryClick} className="text-xs text-gray-400 hover:text-white transition">📸 Galeri</button><span className="text-gray-700">•</span><button onClick={onAdminClick} className="text-xs text-gray-600 hover:text-gray-400 transition">🔒 Panel Panitia</button></div><div className="border-t border-gray-800 pt-4"><p className="text-gray-600 text-xs">© 2026 Panitia HUT RI ke-81 — Perumahan Ciptaland Blok Mawar 🇮🇩</p></div></div></footer>

      {/* FLOATING WA */}
      <div className="fixed bottom-6 right-6 z-50">{waOpen && (<div className="absolute bottom-16 right-0 bg-white rounded-2xl shadow-2xl border w-72 overflow-hidden animate-in mb-2"><div className="bg-green-500 px-4 py-3 text-white"><div className="font-bold text-sm">Hubungi Panitia</div></div>{[{n:'Eka Rista Y',j:'Penanggung Jawab',h:'0821-7129-9984'},{n:'Bayu S.Permana',j:'Ketua Panitia',h:'0812-8839-5550'},{n:'Sugiono',j:'Wakil Ketua',h:'0831-8395-0205'}].map((c,i)=>(<a key={i} href={`https://wa.me/62${c.h.replace(/\D/g,'').replace(/^0/,'')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition border-b last:border-0"><div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-600 text-lg">📱</div><div className="flex-1"><div className="font-bold text-sm text-gray-800">{c.j}</div><div className="text-xs text-gray-500">{c.h}</div></div></a>))}</div>)}<button onClick={() => setWaOpen(!waOpen)} className={`w-14 h-14 rounded-full shadow-2xl flex items-center justify-center text-2xl transition-all hover:scale-110 ${waOpen ? 'bg-red-500 text-white rotate-45' : 'bg-green-500 text-white'}`}>{waOpen ? '✕' : '💬'}</button></div>
