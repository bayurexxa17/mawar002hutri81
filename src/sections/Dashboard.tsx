...
export default function Dashboard() {
  ...
  return (
    <section id="ringkasan" className="py-8 px-4 bg-[#F5F5F0] min-h-screen">
      <div className="max-w-7xl mx-auto">
        {/* Tab Navigation - Like Reference Image */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-2 mb-6 flex flex-wrap gap-2">
          {tabs.map((tab) => (
            <button
              key={tab.id}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[
                      ...
                      { jabatan: 'Ketua Penanggung Jawab', nama: 'Eka Rista Y ' },
                      { jabatan: 'Ketua Panitia', nama: 'Bayu S.Permana (0812-8839-5550)' },
                      { jabatan: 'Wakil Ketua', nama: 'Sugionoa (0831-8395-0205)' },
                      { jabatan: 'Sekretaris', nama: 'Lani (0812-3456-7891)' },
                      { jabatan: 'Bendahara I', nama: 'Aulia Komari (0812-3456-7892' },
                      { jabatan: 'Bendahara II Jawab', nama: 'Puput (0812-3456-7893)' },
                    ].map((row, i) => (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#C1272D] to-[#8B1D20] rounded-2xl p-6 text-white relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-red-100 text-sm mb-6">Transfer ke rekening resmi panitia, lalu isi Form Konfirmasi di bawah agar tercatat.</p>
                
                {/* Bank Cards */}
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-white rounded-xl p-4 text-gray-800">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold">BCA</div>
                      <div className="flex-1">
                        <div className="font-black">BCA - 8465 1234 567</div>
                        <div className="text-xs text-gray-500">a.n. Aulia Komari (Bendahara 1)</div>
                      </div>
                      <button onClick={()=>{
                        navigator.clipboard.writeText('84651234567');
                        alert('No. Rekening BCA disalin: 84651234567');
                      }} className="bg-gray-900 text-white text-xs px-3 py-2 rounded-full hover:bg-black">📋 Salin</button>
                    </div>
                  </div>
                  <div className="bg-white rounded-xl p-4 text-gray-800">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-orange-500 rounded-lg flex items-center justify-center text-white font-bold">BRI</div>
                      <div className="flex-1">
                        <div className="font-black">BRI - 0132 01 123456 50 1</div>
                        <div className="text-xs text-gray-500">a.n. Puput (Bendahara 2)</div>
                      </div>
                      <button onClick={()=>{
                        navigator.clipboard.writeText('013201123456501');
                        alert('No. Rekening BRI disalin');
                      }} className="bg-gray-900 text-white text-xs px-3 py-2 rounded-full hover:bg-black">📋 Salin</button>
                    </div>
                  </div>
                  <div className="bg-white rounded-xl p-4 text-gray-800">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-blue-500 rounded-lg flex items-center justify-center text-white font-bold">DANA</div>
                      <div className="flex-1">
                        <div className="font-black">DANA - 0812-8839-5550</div>
                        <div className="text-xs text-gray-500">a.n. Bayu S.Permana (Ketua)</div>
                      </div>
                      <button onClick={()=>{
                        navigator.clipboard.writeText('081288395550');
                        alert('No. DANA disalin');
                      }} className="bg-gray-900 text-white text-xs px-3 py-2 rounded-full hover:bg-black">📋 Salin</button>
                    </div>
                  </div>
                  <div className="bg-white rounded-xl p-4 text-gray-800">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-green-600 rounded-lg flex items-center justify-center text-white">QRIS</div>
                      <div className="flex-1">
                        <div className="font-black">QRIS All Payment</div>
                        <div className="text-xs text-gray-500">Scan pakai GoPay, OVO, ShopeePay</div>
                      </div>
                      <button onClick={()=>{document.getElementById('qris-detail')?.scrollIntoView({behavior:'smooth'})}} className="bg-gray-900 text-white text-xs px-3 py-2 rounded-full">👁️ Lihat</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 bg-white/10 backdrop-blur-sm rounded-xl p-3 text-xs flex items-center gap-2">
                  <span>💡</span>
                  <span>Edit no. rekening di <code>src/data/banks.ts</code> dan di file ini. Setelah transfer, isi Form Konfirmasi agar tercatat di Daftar Donatur.</span>
                </div>
              </div>
            </div>
